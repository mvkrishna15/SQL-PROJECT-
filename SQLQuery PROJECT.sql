CREATE DATABASE HRDATABASEMANAGEMENT

USE HRDATABASEMANAGEMENT
...
region_id INT NOT NULL,FOREIGN KEY (region_id)  REFERENCES regions (region_id) ON DELETE CASCADE 
ON UPDATE CASCADE) 
...
country_id CHAR (2) NOT NULL,FOREIGN KEY (country_id) REFERENCES countries (country_id) ON DELETE 
...
select employee_id,first_name,last_name,salary*2 as New_salary from employees
...
SELECT TOP 5 first_name,last_name,salary FROM employees ORDER BY first_name
SELECT  TOP 5first_name,last_name,salary FROM employees ORDER BY first_name 
SELECT TOP 5 salary FROM employees ORDER BY salary DESC 
SELECT * FROM employees e1 WHERE 2 = (SELECT COUNT(DISTINCT Salary)FROM employees e2 WHERE e2.Salary >= e1.Salary)
...
select * from employees where salary != '2500' and salary !='2900'
select * from employees where hire_date between '1999-01-01' and '2000-12-31'
select * from employees where hire_date !='1989-01-01' and hire_date !='1999-12-31'
...
select * from employees where department_id!=1 and department_id !=2 and department_id!=3
SELECT *
FROM Employees
WHERE department_id NOT IN (1, 2, 3)
 ...
 select  employees.department_id ,employees .first_name,employees .last_name ,employees.salary ,employees.hire_date
 from employees  inner join departments on employees.department_id= departments.department_id WHERE employees.department_id IN (1, 2, 3) 

 select employees .first_name ,employees .last_name ,employees.department_id ,employees .job_id,job_title 
 from employees inner join jobs on employees.job_id =jobs.job_id inner join departments  on  employees . department_id =departments .department_id where employees.department_id in (1,2,3)

 select countries .country_id,countries .country_name  from countries  left join locations on locations.location_id=countries .country_name

  select regions .region_id ,regions .region_name,countries .country_name ,locations .city
  from regions inner join countries on  regions .region_id=countries.region_id inner join locations on countries .country_id = locations .location_id 
...
select * from fruits as f  FULL OUTER JOIN baskets as b on f.basket_id=b.basket_id
...
SELECT  e.employee_id, e.first_name, e.last_name, e.salary AS EmployeeSalary, (SELECT AVG(salary) FROM employees) AS AverageSalary, e.salary - (SELECT AVG(salary) FROM employees) AS SalaryDifference
FROM  employees e
